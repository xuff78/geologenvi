...
public class ScrollPoints extends LinearLayout {
	...
	public void initPoints(Context context, int count, int selected) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (i == selected) {
				slidePot.setImageResource(R.drawable.slide_pot_selected);
			} else {
				slidePot.setImageResource(R.drawable.slide_pot_unselected);
			}
	...
	public void changeSelectedPoint(int position) {
		for (int i = 0; i < mPoints.size(); i++) {
			...
			if (i == position) {
				point.setImageResource(R.drawable.slide_pot_selected);
			} else {
				point.setImageResource(R.drawable.slide_pot_unselected);
			}
		}
	}
